...
#else
	 #ifndef UNIX_MODE //Revisited by Jun Hu
	 #include <hash_set>
	 #endif
#endif
...
#if XCODE_ENABLE
// the same functionality as imagesc in MATLAB
// input image a should be a gray image
void ImagingScaleData( IplImage * a, char b[] )
{
	if ( !a || a->nChannels > 1 )
	{
		printf( "input of ImagingScaleData() is NULL or not 1 channel" );
		return;
	}
	
	FILE * fr = fopen( "/Users/jh214/Desktop/ABC/colormap.txt", "r" );
	float colormap[64][3];
	
	for ( int index = 0; index < 64; index++ )
	{
		fscanf( fr, "%f %f %f", &colormap[index][0], &colormap[index][1], &colormap[index][2] );
	}
	
	double maxv = -1.7976931E+308, minv = 1.7976931E+308, value;
	int px, py;
	CvScalar s;
	
	for ( py = 0; py < a->height; py ++ )
		for ( px = 0; px < a->width; px++ )
		{
			s = cvGet2D( a, py, px );
			value = s.val[0];
			if ( value > maxv )
	...
	double gap = maxv - minv, epsilon = exp( double( -50 ) );
	
	IplImage * color_img = cvCreateImage( cvGetSize( a ), 32, 3 );
	int step_color = color_img->widthStep / sizeof( float );
	float * cptr = ( float * ) color_img->imageData;
	...
	for ( py = 0; py < a->height; py ++ )
		for ( px = 0; px < a->width; px++ )
		{
			s = cvGet2D( a, py, px );
			value = s.val[0];
			value = ( value - minv ) / ( gap + epsilon );
			int pos = floor( value * 64 );
			cptr[px * 3 + py * step_color] = colormap[63 - pos][0];
			cptr[px * 3 + py * step_color + 1] = colormap[63 - pos][1];
			cptr[px * 3 + py * step_color + 2] = colormap[63 - pos][2];
		}
	
	cvShowImage( b, color_img );
...
VBMP *knn_init_nn(Params *p, BITMAP *a, BITMAP *b, VBMP *&ann_sim, PRINCIPAL_ANGLE *pa, BOOLMAP *mask) {

	if (mask == NULL) {
		mask = create_boolmap(a->w, a->h);
		reset_boolmap(mask);
	} // Added by Jun Hu	
  ...
  if (p->knn_algo == KNN_ALGO_TOP1NN) {
    ...
    for (int y = 0; y < AEH; y++) {
		int seed = rand()^(y<<11);
		bool *row = (bool *) mask->line[y]; 	//Jun Hu	
		for (int x = 0; x < AEW; x++) {						
			if (row[x] == false) // Jun Hu
				continue;		  
...
//#define LAMBDA_PRIOR 0.1 //Added by Jun Hu
...
void knn_attempt_n(vector<qtype<int> > &q, int *adata, BITMAP *b, XFORM bpos, int bx, int by, int bs, int bt, Params *p, int dval_known, PositionSet &pos0, int bx_prior = INT_MAX, int by_prior = INT_MAX, double lambda = 0, int truncated_d = INT_MAX) {              
...
#if TRANSLATE_ONLY
      current = fast_patch_dist<PATCH_W, 0>(adata, b, bpos.x0>>16, bpos.y0>>16, err, p, truncated_d);
#else
      current = sim_fast_patch_dist<PATCH_W, 1>(adata, b, bpos, err);
#endif
      //Revisted by Jun Hu, Add Prior Term
      int bx_penalty = bx_prior - bx;
      int by_penalty = by_prior - by;
      char buf[256];
      sprintf(buf, "before: %d", current);
	  //int lambda = 0; //Added by Jun Hu
      //if (bx_prior != INT_MAX & by_prior != INT_MAX)
	  //	  lambda = 3*LAMBDA_PRIOR*PATCH_W*PATCH_W; 				
      lambda = 3*lambda*PATCH_W*PATCH_W; 
	  if (bx_prior == INT_MAX || by_prior == INT_MAX)
		  lambda = 0;
      current += int(lambda*bx_penalty*bx_penalty + lambda*by_penalty*by_penalty);
      sprintf(buf, "after: %d", current);      
...
//VBMP *knn_init_dist_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim) { //Revistied by Jun Hu, Add Prior Term
VBMP *knn_init_dist_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) {
	...
	//int lambda = LAMBDA_PRIOR*PATCH_W*PATCH_W; //Added by Jun Hu
	lambda = lambda*PATCH_W*PATCH_W;
...
#pragma omp parallel for schedule(static,4)
		for (int y = 0; y < AEH; y++) {
			...
			for (int x = 0; x < AEW; x++) {
				...
				//int d = patch_dist_ab<PATCH_W, 0, 0>(p, a, x, y, b, bx, by, INT_MAX, NULL); Revisited by Jun Hu
				int d = patch_dist_ab<PATCH_W, 0, 0>(p, a, x, y, b, bx, by, INT_MAX, NULL, truncated_d);
				
				//Revisted by Jun Hu, Add Prior Term
				if(ann_prior != NULL){
					int *b_prior = ann_prior->get(x,y);
					int bx_penalty= INT_TO_X(*b_prior) - bx;
					int by_penalty = INT_TO_Y(*b_prior) - by;
					d += int(lambda*bx_penalty*bx_penalty + lambda*by_penalty*by_penalty);
				}
...
#pragma omp parallel for schedule(static,4)
		for (int y = 0; y < AEH; y++) {
			...
			for (int x = 0; x < AEW; x++) {
				...
				//Revisited by Jun Hu
				int bx_prior = INT_MAX, by_prior = INT_MAX;
				if (ann_prior != NULL) {
					int *b_prior = ann_prior->get(x,y);
					bx_prior = INT_TO_X(b_prior[0]), by_prior = INT_TO_Y(b_prior[0]);
				}
...
VBMP *knn_init_dist(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) {
	if      (p->patch_w == 1 ) { return knn_init_dist_n<1>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 2 ) { return knn_init_dist_n<2>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 3 ) { return knn_init_dist_n<3>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 4 ) { return knn_init_dist_n<4>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 5 ) { return knn_init_dist_n<5>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 6 ) { return knn_init_dist_n<6>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 7 ) { return knn_init_dist_n<7>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 8 ) { return knn_init_dist_n<8>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 9 ) { return knn_init_dist_n<9>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 10) { return knn_init_dist_n<10>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 11) { return knn_init_dist_n<11>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 12) { return knn_init_dist_n<12>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 13) { return knn_init_dist_n<13>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 14) { return knn_init_dist_n<14>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 15) { return knn_init_dist_n<15>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 16) { return knn_init_dist_n<16>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 17) { return knn_init_dist_n<17>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 18) { return knn_init_dist_n<18>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 19) { return knn_init_dist_n<19>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 20) { return knn_init_dist_n<20>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 21) { return knn_init_dist_n<21>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 22) { return knn_init_dist_n<22>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 23) { return knn_init_dist_n<23>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 24) { return knn_init_dist_n<24>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 25) { return knn_init_dist_n<25>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 26) { return knn_init_dist_n<26>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 27) { return knn_init_dist_n<27>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 28) { return knn_init_dist_n<28>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 29) { return knn_init_dist_n<29>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 30) { return knn_init_dist_n<30>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 31) { return knn_init_dist_n<31>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
	else if (p->patch_w == 32) { return knn_init_dist_n<32>(p, a, b, ann, ann_sim, ann_prior,  lambda,  truncated_d, mask); }
...
void change_knn(Params *p, BITMAP *a, BITMAP *b, VBMP *&ann0, VBMP *&ann_sim0, VBMP *&annd0, int kp, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d) { // Revisited by Jun Hu, Add Prior Term
  if (kp == p->knn) { return; }
...
/*
    ...
    */
    ...
    VBMP *ann_sim = NULL;
#endif
    annd = knn_init_dist(&pnew, a, b, ann, ann_sim, ann_prior, lambda, truncated_d);
  }
...
//            RegionMasks *region_masks=NULL, int tiles=-1, PRINCIPAL_ANGLE *pa=NULL, int save_first=1) {
              RegionMasks *region_masks=NULL, int tiles=-1, PRINCIPAL_ANGLE *pa=NULL, int save_first=1, VBMP *ann_prior=NULL, double lambda = 0, int truncated_d = INT_MAX, BOOLMAP *mask = NULL) { // Revisited by Jun HU, Add Prior Term
...
#if XCODE_ENABLE        
IplImage *flowx = cvCreateImage( cvSize(a->w-PATCH_W, a->h-PATCH_W), sizeof(int)*8, 1);
float *p_flowx = (float*) flowx->imageData;
int step = flowx->widthStep / sizeof(float);
for (int py = 0; py < a->h - PATCH_W ; py++) {
	for (int px = 0; px < a->w - PATCH_W; px++) {
		int *p_ann = ann->get(px, py);
		p_flowx[px + py*step] = INT_TO_X(p_ann[0]) - px;
	}
}

ImagingScaleData( flowx, "before: flowx" );  
#endif
...
//          v.clear();
         ...
         //Revisited by Jun Hu
         int xp_prior = INT_MAX, yp_prior = INT_MAX;
         if (ann_prior != NULL) {
            int *p_prior = ann_prior->get(x,y);
			xp_prior = INT_TO_X(p_prior[0]), yp_prior = INT_TO_Y(p_prior[0]);
         }
          ...
          for (int j = 0; j < p->knn; j++) {
            ...
            for (int k = 0; k < p->knn; k++) {
              ...
              knn_attempt_n<PATCH_W, 0>(v, adata, b, bpos, xpp, ypp, spp, tpp, p, 0, pos, xp_prior, yp_prior, lambda, truncated_d); // Revisited by Jun Hu, Add Prior Term
...
//         RegionMasks *region_masks, int tiles, PRINCIPAL_ANGLE *pa, int save_first) {
           RegionMasks *region_masks, int tiles, PRINCIPAL_ANGLE *pa, int save_first, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) { //Revisited by Jun Hu, Add Prior Term
  ...
  for (int restart = 0; restart < p->restarts; restart++) {
    if (restart > 0) {
	  ...
	  ann = ann_temp = knn_init_nn(p, a, b, ann_sim, NULL, mask); //Revisited by Jun Hu
#if TRANSLATE_ONLY
      ...
      annd = annd_temp = knn_init_dist(p, a, b, ann, ann_sim, ann_prior, lambda, truncated_d, mask); // Revisited by Jun Hu, Add Prior Term 
    }

    if (p->knn_algo == KNN_ALGO_HEAP) {
		if      (p->patch_w == 1) { knn_n<1,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 2) { knn_n<2,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 3) { knn_n<3,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 4) { knn_n<4,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 5) { knn_n<5,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 6) { knn_n<6,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 7) { knn_n<7,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 8) { knn_n<8,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 9) { knn_n<9,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 10) { knn_n<10,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 11) { knn_n<11,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 12) { knn_n<12,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 13) { knn_n<13,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 14) { knn_n<14,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 15) { knn_n<15,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 16) { knn_n<16,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 17) { knn_n<17,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 18) { knn_n<18,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 19) { knn_n<19,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 20) { knn_n<20,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 21) { knn_n<21,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 22) { knn_n<22,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 23) { knn_n<23,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 24) { knn_n<24,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 25) { knn_n<25,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 26) { knn_n<26,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 27) { knn_n<27,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 28) { knn_n<28,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 29) { knn_n<29,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 30) { knn_n<30,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 31) { knn_n<31,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
		else if (p->patch_w == 32) { knn_n<32,0>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); }
      else { fprintf(stderr, "Patch size unsupported for knn_oneway: %d\n", p->patch_w); exit(1); }
    } else
    { fprintf(stderr, "others are not implemented in mex code\n"); exit(1);}
//     } else if (p->knn_algo == KNN_ALGO_PRINANGLE) {
//       if      (p->patch_w == 1) { knn_n<1,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 2) { knn_n<2,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 3) { knn_n<3,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 4) { knn_n<4,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 5) { knn_n<5,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 6) { knn_n<6,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 7) { knn_n<7,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 8) { knn_n<8,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 9) { knn_n<9,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 10) { knn_n<10,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 11) { knn_n<11,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 12) { knn_n<12,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 13) { knn_n<13,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 14) { knn_n<14,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 15) { knn_n<15,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 16) { knn_n<16,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 17) { knn_n<17,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 18) { knn_n<18,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 19) { knn_n<19,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 20) { knn_n<20,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 21) { knn_n<21,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 22) { knn_n<22,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 23) { knn_n<23,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 24) { knn_n<24,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 25) { knn_n<25,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 26) { knn_n<26,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 27) { knn_n<27,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 28) { knn_n<28,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 29) { knn_n<29,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 30) { knn_n<30,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 31) { knn_n<31,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else if (p->patch_w == 32) { knn_n<32,1>(p, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior); }
//       else { fprintf(stderr, "Patch size unsupported for knn_oneway: %d\n", p->patch_w); exit(1); }
...
void knn(Params *p, BITMAP *a, BITMAP *b,
         ...
         int level, int em_iter, RecomposeParams *rp, int offset_iter, int update_type, int cache_b,
//         RegionMasks *region_masks, int tiles, PRINCIPAL_ANGLE *pa, int save_first) {
        RegionMasks *region_masks, int tiles, PRINCIPAL_ANGLE *pa, int save_first, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) { // Revisted by Jun Hu, Prior Term
  ...
  if (p->knn_algo == KNN_ALGO_CHANGEK) {
    ...
    knn(&pcopy, a, b, ann, ann_sim, annd, amask, bmask, level, em_iter, rp, offset_iter, update_type, cache_b, region_masks, tiles, pa, save_first, ann_prior, lambda, truncated_d, mask); // Revisited by Jun HU, Add Prior Term
    
    fprintf(stderr, "KNN_ALGO_CHANGEK, second part, changing k to %d\n", change_k_to);
    //change_knn(&pcopy, a, b, ann, ann_sim, annd, change_k_to, pa);
    change_knn(&pcopy, a, b, ann, ann_sim, annd, change_k_to, pa, ann_prior, truncated_d); //Revisited by Jun Hu, Prior Term
...
#if XCODE_ENABLE
IplImage *flowx = cvCreateImage( cvSize(a->w, a->h), sizeof(int)*8, 1);
float *p_flowx = (float*) flowx->imageData;
int step = flowx->widthStep / sizeof(float);
for (int py = 0; py < a->h ; py++) {
  for (int px = 0; px < a->w; px++) {
	  int *p_ann = ann->get(px, py);
	  p_flowx[px + py*step] = INT_TO_X(p_ann[0]);
  }
}
...
ImagingScaleData( flowx, "before: flowx" );
    ...
    knn_oneway(p, a, b, ann, ann_sim, annd, NULL, NULL, 0, 0, NULL, 0, 0, 0, NULL, -1, pa, iter == 0, ann_prior, lambda, truncated_d, mask); // Revisited by Jun Hu, Add Prior Term
	  ...
	  ImagingScaleData( flowx, "after: flowx" ); */
      ...
      for (int enrich_i = 0; enrich_i < p->enrich_times; enrich_i++) {
        ...
        if (p->do_inverse_enrich) {
          if (inverse_enrich_hops == 1) {
            knn_inverse_enrich(p, a, a, ann, ann_sim, annd, pa, mask);
          } else if (inverse_enrich_hops == 2) {
            //knn_inverse_enrich2(p, a, a, ann, ann_sim, annd, pa);
             knn_inverse_enrich2(p, a, a, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); // Revisited by Jun Hu, Add Prior Term 
          } else {
        ...
        if (p->do_enrich) {
          if (forward_enrich_hops == 2) {              
            ...
            knn_enrich(p, a, a, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask);  // Revisited by Jun Hu, Add Prior Term 
          } else if (forward_enrich_hops == 3) {
            //knn_enrich3(p, a, a, ann, ann_sim, annd, pa);
            knn_enrich3(p, a, a, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask);  // Revisited by Jun Hu, Add Prior Term  
          } else if (forward_enrich_hops == 4) {
            //knn_enrich4(p, a, a, ann, ann_sim, annd, pa);
            knn_enrich4(p, a, a, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask);  // Revisited by Jun Hu, Add Prior Term  
          } else {
...
void knn_enrich_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) { //Revisited by Jun Hu, Add Prior Term        
    ...
    for (int x = box.xmin; x < box.xmax; x++) {
      ...
      for (int dy0 = 0; dy0 < PATCH_W; dy0++) {
        ...
        for (int dx0 = 0; dx0 < PATCH_W; dx0++) {
         ...
         if (ann_prior != NULL) {
            int *p_prior = ann_prior->get(x,y);
            int xp_prior = INT_TO_X(p_prior[0]), yp_prior = INT_TO_Y(p_prior[0]);
         } 
...
void knn_enrich(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) {
	if      (p->patch_w == 1 ) { return knn_enrich_n<1 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 2 ) { return knn_enrich_n<2 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 3 ) { return knn_enrich_n<3 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 4 ) { return knn_enrich_n<4 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 5 ) { return knn_enrich_n<5 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 6 ) { return knn_enrich_n<6 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 7 ) { return knn_enrich_n<7 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 8 ) { return knn_enrich_n<8 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 9 ) { return knn_enrich_n<9 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 10) { return knn_enrich_n<10>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 11) { return knn_enrich_n<11>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 12) { return knn_enrich_n<12>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 13) { return knn_enrich_n<13>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 14) { return knn_enrich_n<14>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 15) { return knn_enrich_n<15>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 16) { return knn_enrich_n<16>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 17) { return knn_enrich_n<17>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 18) { return knn_enrich_n<18>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 19) { return knn_enrich_n<19>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 20) { return knn_enrich_n<20>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 21) { return knn_enrich_n<21>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 22) { return knn_enrich_n<22>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 23) { return knn_enrich_n<23>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 24) { return knn_enrich_n<24>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 25) { return knn_enrich_n<25>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 26) { return knn_enrich_n<26>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 27) { return knn_enrich_n<27>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 28) { return knn_enrich_n<28>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 29) { return knn_enrich_n<29>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 30) { return knn_enrich_n<30>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 31) { return knn_enrich_n<31>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 32) { return knn_enrich_n<32>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
  else { fprintf(stderr, "Patch width unsupported: %d\n", p->patch_w); exit(1); }
}
...
void knn_enrich3_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) { //Revisited by Jun Hu, Add Prior Term       
...
void knn_enrich3(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) {
	if      (p->patch_w == 1 ) { return knn_enrich3_n<1 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 2 ) { return knn_enrich3_n<2 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 3 ) { return knn_enrich3_n<3 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 4 ) { return knn_enrich3_n<4 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 5 ) { return knn_enrich3_n<5 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 6 ) { return knn_enrich3_n<6 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 7 ) { return knn_enrich3_n<7 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 8 ) { return knn_enrich3_n<8 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 9 ) { return knn_enrich3_n<9 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 10) { return knn_enrich3_n<10>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 11) { return knn_enrich3_n<11>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 12) { return knn_enrich3_n<12>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 13) { return knn_enrich3_n<13>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 14) { return knn_enrich3_n<14>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 15) { return knn_enrich3_n<15>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 16) { return knn_enrich3_n<16>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 17) { return knn_enrich3_n<17>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 18) { return knn_enrich3_n<18>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 19) { return knn_enrich3_n<19>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 20) { return knn_enrich3_n<20>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 21) { return knn_enrich3_n<21>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 22) { return knn_enrich3_n<22>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 23) { return knn_enrich3_n<23>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 24) { return knn_enrich3_n<24>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 25) { return knn_enrich3_n<25>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 26) { return knn_enrich3_n<26>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 27) { return knn_enrich3_n<27>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 28) { return knn_enrich3_n<28>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 29) { return knn_enrich3_n<29>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 30) { return knn_enrich3_n<30>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 31) { return knn_enrich3_n<31>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 32) { return knn_enrich3_n<32>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
  else { fprintf(stderr, "Patch width unsupported: %d\n", p->patch_w); exit(1); }
}

template<int PATCH_W>
//void knn_enrich4_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa) {
void knn_enrich4_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) { //Revisited by Jun Hu, Add Prior Term
...
void knn_enrich4(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) {
	if      (p->patch_w == 1 ) { return knn_enrich4_n<1 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 2 ) { return knn_enrich4_n<2 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 3 ) { return knn_enrich4_n<3 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 4 ) { return knn_enrich4_n<4 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 5 ) { return knn_enrich4_n<5 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 6 ) { return knn_enrich4_n<6 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 7 ) { return knn_enrich4_n<7 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 8 ) { return knn_enrich4_n<8 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 9 ) { return knn_enrich4_n<9 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 10) { return knn_enrich4_n<10>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 11) { return knn_enrich4_n<11>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 12) { return knn_enrich4_n<12>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 13) { return knn_enrich4_n<13>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 14) { return knn_enrich4_n<14>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 15) { return knn_enrich4_n<15>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 16) { return knn_enrich4_n<16>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 17) { return knn_enrich4_n<17>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 18) { return knn_enrich4_n<18>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 19) { return knn_enrich4_n<19>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 20) { return knn_enrich4_n<20>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 21) { return knn_enrich4_n<21>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 22) { return knn_enrich4_n<22>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 23) { return knn_enrich4_n<23>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 24) { return knn_enrich4_n<24>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 25) { return knn_enrich4_n<25>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 26) { return knn_enrich4_n<26>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 27) { return knn_enrich4_n<27>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 28) { return knn_enrich4_n<28>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 29) { return knn_enrich4_n<29>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 30) { return knn_enrich4_n<30>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 31) { return knn_enrich4_n<31>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 32) { return knn_enrich4_n<32>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
  else { fprintf(stderr, "Patch width unsupported: %d\n", p->patch_w); exit(1); }
}
...
template<int PATCH_W>
void knn_inverse_enrich_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, BOOLMAP *mask) {
...
void knn_inverse_enrich(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, BOOLMAP *mask) {
	if      (p->patch_w == 1 ) { return knn_inverse_enrich_n<1 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 2 ) { return knn_inverse_enrich_n<2 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 3 ) { return knn_inverse_enrich_n<3 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 4 ) { return knn_inverse_enrich_n<4 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 5 ) { return knn_inverse_enrich_n<5 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 6 ) { return knn_inverse_enrich_n<6 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 7 ) { return knn_inverse_enrich_n<7 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 8 ) { return knn_inverse_enrich_n<8 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 9 ) { return knn_inverse_enrich_n<9 >(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 10) { return knn_inverse_enrich_n<10>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 11) { return knn_inverse_enrich_n<11>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 12) { return knn_inverse_enrich_n<12>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 13) { return knn_inverse_enrich_n<13>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 14) { return knn_inverse_enrich_n<14>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 15) { return knn_inverse_enrich_n<15>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 16) { return knn_inverse_enrich_n<16>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 17) { return knn_inverse_enrich_n<17>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 18) { return knn_inverse_enrich_n<18>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 19) { return knn_inverse_enrich_n<19>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 20) { return knn_inverse_enrich_n<20>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 21) { return knn_inverse_enrich_n<21>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 22) { return knn_inverse_enrich_n<22>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 23) { return knn_inverse_enrich_n<23>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 24) { return knn_inverse_enrich_n<24>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 25) { return knn_inverse_enrich_n<25>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 26) { return knn_inverse_enrich_n<26>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 27) { return knn_inverse_enrich_n<27>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 28) { return knn_inverse_enrich_n<28>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 29) { return knn_inverse_enrich_n<29>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 30) { return knn_inverse_enrich_n<30>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 31) { return knn_inverse_enrich_n<31>(p, a, b, ann, ann_sim, annd, pa, mask); }
	else if (p->patch_w == 32) { return knn_inverse_enrich_n<32>(p, a, b, ann, ann_sim, annd, pa, mask); }
  else { fprintf(stderr, "Patch width unsupported: %d\n", p->patch_w); exit(1); }
}
...
void knn_inverse_enrich2_n(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) { // Revisited by Jun Hu, Add Prior Term        
...
#endif
        //knn_attempt_n<PATCH_W, 0>(v, adata, b, bpos, xpp, ypp, spp, tpp, p, 0, pos);
        knn_attempt_n<PATCH_W, 0>(v, adata, b, bpos, xpp, ypp, spp, tpp, p, 0, pos, xp_prior, yp_prior, lambda, truncated_d); // Revisited by Jun Hu, Prior Term
        current = current->next;
      }
...
void knn_inverse_enrich2(Params *p, BITMAP *a, BITMAP *b, VBMP *ann, VBMP *ann_sim, VBMP *annd, PRINCIPAL_ANGLE *pa, VBMP *ann_prior, double lambda, int truncated_d, BOOLMAP *mask) {
	if      (p->patch_w == 1 ) { return knn_inverse_enrich2_n<1 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 2 ) { return knn_inverse_enrich2_n<2 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 3 ) { return knn_inverse_enrich2_n<3 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 4 ) { return knn_inverse_enrich2_n<4 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 5 ) { return knn_inverse_enrich2_n<5 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 6 ) { return knn_inverse_enrich2_n<6 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 7 ) { return knn_inverse_enrich2_n<7 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 8 ) { return knn_inverse_enrich2_n<8 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 9 ) { return knn_inverse_enrich2_n<9 >(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 10) { return knn_inverse_enrich2_n<10>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 11) { return knn_inverse_enrich2_n<11>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 12) { return knn_inverse_enrich2_n<12>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 13) { return knn_inverse_enrich2_n<13>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 14) { return knn_inverse_enrich2_n<14>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 15) { return knn_inverse_enrich2_n<15>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 16) { return knn_inverse_enrich2_n<16>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 17) { return knn_inverse_enrich2_n<17>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 18) { return knn_inverse_enrich2_n<18>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 19) { return knn_inverse_enrich2_n<19>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 20) { return knn_inverse_enrich2_n<20>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 21) { return knn_inverse_enrich2_n<21>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 22) { return knn_inverse_enrich2_n<22>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 23) { return knn_inverse_enrich2_n<23>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 24) { return knn_inverse_enrich2_n<24>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 25) { return knn_inverse_enrich2_n<25>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 26) { return knn_inverse_enrich2_n<26>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 27) { return knn_inverse_enrich2_n<27>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 28) { return knn_inverse_enrich2_n<28>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 29) { return knn_inverse_enrich2_n<29>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 30) { return knn_inverse_enrich2_n<30>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 31) { return knn_inverse_enrich2_n<31>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
	else if (p->patch_w == 32) { return knn_inverse_enrich2_n<32>(p, a, b, ann, ann_sim, annd, pa, ann_prior, lambda, truncated_d, mask); }
  else { fprintf(stderr, "Patch width unsupported: %d\n", p->patch_w); exit(1); }
}
