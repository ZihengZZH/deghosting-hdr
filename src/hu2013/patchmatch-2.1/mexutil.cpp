...
BITMAP *convert_field(Params *p, const mxArray *A, int bw, int bh, int &nclip, int trim_patch) {
  ...
  //if (mxGetDimensions(A)[2] != 3) { mexErrMsgTxt("field channels != 3"); } // Revisited by Jun Hu
...
// stack Mask to a boolmap
BOOLMAP *convert_boolmap(const mxArray *A) {
	if (mxGetNumberOfDimensions(A) != 2) { mexErrMsgTxt("dims != 2"); }
	
	// matlab, h * w : (y, x) -> y + x * h
	int h = mxGetDimensions(A)[0];
	int w = mxGetDimensions(A)[1];
	
	if (mxIsUint8(A)) {
		unsigned char *data = (unsigned char *) mxGetData(A);
		BOOLMAP *ans = create_boolmap(w, h);
		for (int y = 0; y < h; y++) {
			bool *row = (bool *) ans->line[y];
			for (int x = 0; x < w; x++) {
				unsigned char *base = data + y + x * h;
				int r = base[0];
				if (base[0] == 1) {
